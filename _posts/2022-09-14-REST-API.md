...
이제 API를 어떻게 활용할지 감을 잡았으니, 본격적으로 async/await을 통해 비동기 함수인 fetch을 사용하여 API를 호출해 보자.
...
기존에 진행중이었던 프로젝트에서 API 호출 공부만을 목적으로 코드를 복사해 온 것이라 코드를 많이 생략했기 때문에 이를 전부 이해할 필요는 없다.
